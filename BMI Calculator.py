...
def BMI():
    ...
    if bmi<18.5:
        ...
        label3.config(text="It indicates that the person is slighly overweight!")
    else:
        label2.config(text="obese!")
        label3.config(text="You need to loose weight!")
...
slider=ttk.Scale(root,from_=0, to=220, orient='horizontal',style="Tscale.Horizontal.TScale",command=slider_changed,variable=current_value)
...
style=ttk.Style()
style.configure("Tscale.Horizontal.TScale",background="white")
slider=ttk.Scale(root,from_=0, to=220, orient='horizontal',style="Tscale.Horizontal.TScale",command=slider_changed2,variable=current_value2)
...
# Height Label
Label(root, text="Height (cm)", font="arial 15", bg="#f0f1f5").place(x=75, y=120)

# Weight Label
Label(root, text="Weight (kg)", font="arial 15", bg="#f0f1f5").place(x=295, y=120)
